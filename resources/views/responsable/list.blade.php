...
<div class="max-w-7xl mx-auto">
    ...
    <div class="flex flex-col xl:flex-row xl:items-start justify-between gap-6 mb-8">
        ...
        <form method="GET" action="{{ route('responsable.salles.list') }}" class="flex flex-wrap items-center gap-2 shrink-0 border border-gray-200 p-2 rounded-xl bg-gray-50/50">
            <!-- Salle Filter -->
            ...
            <!-- Statut Filter -->
            <select name="statut" class="px-3 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-200 rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-green-500 h-10 transition-colors">
            ...
            @if(request()->filled('salle_id') || request()->filled('date') || request()->filled('statut'))
                <a href="{{ route('responsable.salles.list') }}" class="inline-flex items-center justify-center px-4 py-2 bg-red-50 hover:bg-red-100 text-red-600 border border-red-200 text-sm font-bold rounded-lg shadow-sm transition-colors h-10">
                    Effacer
                </a>
            @endif
