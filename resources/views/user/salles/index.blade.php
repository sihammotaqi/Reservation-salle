        ...
        @forelse($salles as $salle)
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-5 flex flex-col justify-between hover:shadow-md transition">
                <div>
                    ...
                    {{-- Mini Timeline --}}
                    @if($salle->plannings->count() > 0)
                        <div class="mb-4">
                            <p class="text-xs text-gray-400 mb-1.5 font-medium">Créneaux aujourd'hui</p>
                            <div class="space-y-1">
                                @foreach($salle->plannings as $p)
                                    <div class="flex items-center gap-2 text-xs">
                                        <span class="w-2 h-2 rounded-full flex-shrink-0
                                            {{ $p->statut === 'approuve' ? 'bg-red-400' : 'bg-orange-400' }}">
                                        </span>
                                        <span class="font-medium text-gray-600">
                                            {{ $p->date_debut->format('H:i') }} – {{ $p->date_fin->format('H:i') }}
                                        </span>
                                        <span class="text-gray-400">{{ $p->type_evenement ?? $p->titre }}</span>
                                    </div>
                                @endforeach
                            </div>
                        </div>
    ...
    <div class="bg-white rounded-2xl shadow-xl w-full max-w-md p-6 relative">
        ...
        <form method="POST" action="{{ route('user.reservations.store') }}">
            ...
            <div class="mb-4">
                ...
                <input type="text" name="titre" value="{{ old('titre') }}" required placeholder="Ex: Réunion d'équipe"
                    class="w-full border border-gray-200 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-green-400">
